...
/**
 * Generate motion curve points from AI insights
 */
function generateMotionCurve(clip: Clip, pointCount: number = 60): number[] {
  const { motionScore, shotStability, cinematicRhythm } = clip.analysis;

  // Normalize scores to 0-1 range (assuming they're 0-100)
  const normalizedMotion = motionScore / 100;
  const normalizedStability = shotStability / 100;
  const normalizedRhythm = cinematicRhythm / 100;

  // Base motion value from weighted AI insights
  // Higher motion, lower stability = more dynamic movement
  const baseMotion =
    normalizedMotion * 0.6 + (1 - normalizedStability) * 0.25 + normalizedRhythm * 0.15;
  ...
  for (let i = 0; i < pointCount; i++) {
    ...
    // Create natural wave pattern (more erratic than emotion)
    const wave1 = Math.sin(progress * Math.PI * 3) * 0.2;
    const wave2 = Math.cos(progress * Math.PI * 5 + 0.5) * 0.12;

    // Add slight random jitter (±4% - more than emotion)
    const jitter = (Math.random() - 0.5) * 0.08;

    // Combine base + waves + jitter
    let value = baseMotion + wave1 + wave2 + jitter;
...
/**
 * MotionCurve - Displays a cinematic motion activity timeline
 */
export default function MotionCurve({
  ...
  if (!clip || !clip.analysis) {
    if (isDev) {
      console.warn('MotionCurve: Missing clip or analysis data', {
        hasClip: !!clip,
  ...
  if (
    ...
    typeof shotStability !== 'number' ||
    typeof cinematicRhythm !== 'number'
  ) {
    if (isDev) {
      console.warn('MotionCurve: Invalid analysis scores', {
        clipId: clip.id,
        motionScore,
        shotStability,
        cinematicRhythm,
  ...
  if (typeof startTime !== 'number' || typeof endTime !== 'number' || startTime >= endTime) {
    if (isDev) {
      console.warn('MotionCurve: Invalid time range', { startTime, endTime });
    }
    return null;
  }

  // Generate motion curve points
  // regenerateKey is intentionally included to force re-generation when clip regenerates
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const motionPoints = useMemo(() => {
    return generateMotionCurve(clip, 60);
  }, [clip, regenerateKey]);
  ...
  const pathData = useMemo(() => {
    ...
    const height = 120;
    return createSmoothPath(motionPoints, width, height);
  }, [motionPoints]);
    ...
    <div className="w-full h-[120px] relative overflow-hidden bg-black/20 border-y border-mono-silver/10">
      ...
      <div className="absolute inset-0 opacity-[0.04]">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="motion-grid" width="40" height="20" patternUnits="userSpaceOnUse">
      ...
      >
        ...
        <path
          ...
          className="drop-shadow-[0_0_8px_rgba(0,255,255,0.3)]"
        />
      </svg>
      ...
      {/* Label */}
      <div className="absolute top-2 left-4 text-[10px] font-montserrat font-semibold text-cyan-400/60 uppercase tracking-wider">
        Motion Activity
      </div>
    </div>
  );
